...
class Game(arcade.Window):
    def __init__(self):
        arcade.Window.__init__(self, WIN_WIDTH, WIN_HEIGHT, "AI Snake by Benyamin Zojaji")
    ...
    def on_update(self, delta_time: float):
        ai_move = self.Ai.predict(self.snake.center_x, self.snake.center_y,
                                 self.apple.apple.center_x, self.apple.apple.center_y,
                                 self.snake.center_x - self.apple.apple.center_x, self.snake.center_y - self.apple.apple.center_y)
        if ai_move == 0:
